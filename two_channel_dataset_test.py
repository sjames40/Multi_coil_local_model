...
#data_path = '/home/liangs16/labmat_project/MRI_descattering/NEW_8_fold'
#os.chdir(rt)
#foldername = []
#for i in os.listdir():
#    if i.endswith('h5'):
...
for a in range(len(kspace_array)):
    norms = np.linalg.norm(torch.view_as_complex(image_space_gh[1180].permute(1,2,0))-torch.view_as_complex(image_space_gh[a].permute(1,2,0)))# norm matrix x is clean patch_image
   # norms2 =np.linalg.norm(np.abs(image_space_gh[1100])-np.abs(image_space_gh[a]))
    ...
    #norms = np.linalg.norm(test[0][0]-clean_data[a],'fro')
    #norm_matrix.append(norms)
    norm_matrix2.append(norms)
#match_inds = np.argsort(norm_matrix)[1:40+1]
...
class nyumultidataset(Dataset):
    ...
    def __getitem__(self, index):
       ...
       # A_temp = h5py.File(A_path, 'r')
        ...
        A_I_mask = A_I_mask[:, nx//2-self.nx//2:nx//2+self.nx//2,ny//2-self.ny//2:ny//2+self.ny//2]
        A_I = center_crop(A_I, (320,320))
        A_I_mask = center_crop(A_I_mask, (320,320)) 
...
#    np.save(os.path.join('..','MRI_descattering','Test_image','groundturth_2channel.npy'),direct.detach().numpy())
#    np.save(os.path.join('..','MRI_descattering','Test_image','undersample_2channel.npy'),target.detach().numpy())
