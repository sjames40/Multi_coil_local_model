...
#            out_chans=2)
# netG = UNet().to(device)
netG =Unet(2,2, num_pool_layers=4, chans=32).to(device)
netG = netG.float()
...
A_I = np.load(os.path.join('..','MRI_descattering','two_channel_result','vali_gh_8_fold_ch1180_l7.npy'))
A_I_mask = np.load(os.path.join('..','MRI_descattering','two_channel_result','vali_noise_8_fold_ch1180_l7.npy'))
netG = torch.load(os.path.join('..','MRI_descattering','Unet_two_channel_model_1400_image_4accelreat_corr3.pt'))
A_I_local =np.load(os.path.join('..','MRI_descattering','two_channel_result','vali_image_8_fold_ch1180_l7.npy'))
with torch.no_grad():
    ...
    val_local2 = convert_2chan_into_abs(vali_local).numpy()
    val_pred2 = convert_2chan_into_abs(val_pred).numpy()
    vali_target2 = convert_2chan_into_abs(vali_target).numpy()
    vali_input2 =convert_2chan_into_abs(vali_input).numpy()
    #print(val_local.shape)
    print('input psnr', PSNR2(np.abs(vali_input2), np.abs(vali_target2)))
    print('global recons psnr', PSNR2(np.abs(val_pred2), np.abs(vali_target2)))
    print('local recons psnr', PSNR2(np.abs(val_local2), np.abs(vali_target2)))
...
np.save(os.path.join('..','MRI_descattering','two_channel_result','vali_image_global_4fold_ch1180.npy'),val_pred)
